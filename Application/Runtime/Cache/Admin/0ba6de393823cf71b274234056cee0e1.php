	...
	<table>
        ...
        <tbody>
            <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol): $mod = ($i % 2 );++$i;?><tr>
                ...
                <td class="name"><?php echo (str_repeat('&emsp;',$vol["level"]*3)); echo ($vol["name"]); ?></td>
                <td class="process"><?php if($vol["pid"] == 0): ?>顶级部门<?php else: echo ($vol["pname"]); endif; ?></td>
                <td class="node"><?php echo ($vol["sort"]); ?></td>
                <td class="time"><?php echo ($vol["remark"]); ?></td>
                <td class="operate" >
                    <input  style="vertical-align: middle" type="checkbox" value="<?php echo ($vol["id"]); ?>">
...
$(function(){
    ...
    $('.del').on('click',function(){
        //事件处理程序
        var box = $(':checkbox:checked');//获取全部被选中的复选框
