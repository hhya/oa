...
<table>
    ...
    <tbody>
        <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol): $mod = ($i % 2 );++$i;?><tr>
                ...
                <td class="content"><?php echo (msubstr($vol["content"],0,6)); ?></td>
                <td class="addtime"><?php echo (date('Y-m-d H:i:s',$vol["addtime"])); ?></td>
                <td class="status"><?php if($vol["isread"] == 1): ?><span style='color: green'>已读</span><?php else: ?><span style='color: red'>未读</span><?php endif; ?></td>
                <td class="operate">
                	<a href ='javascript:;' class='show' data_id='<?php echo ($vol["id"]); ?>' data_title='<?php echo ($vol["title"]); ?>' status='<?php echo ($vol["isread"]); ?>'>查看</a> 
                </td>
